...
import { emailInput, btnSignIn, signInMain, passwordInput } from "../helpers/selectorsSignInPage";
...
import { btnManageProfile, rememberMe } from "../helpers/selectorsSignInPage";
...
test.describe("Positive tests on Netflix/by", () => {
    ...
    test("Check the URL to contain netflix", async ({ page }) => {
        await expect(page.url()).toContain(netflixExpected);
    });
    test("Check the Title of Netflix website", async ({ page }) => {
        await expect(page).toHaveTitle(titleExpected);
    });
    test("Check Sign In button links to correct page", async ({ page }) => {
